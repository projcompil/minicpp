...
retour=0
outil0="spim -f"
outil1="java -jar /Users/othmane/Desktop/Mars4_4.jar"
...
function compi {
        ...
        if [ $? == 0 ] ; then
                if [ $2 == 0 ] ; then
                        ...
                        $outil1 "$pathstore$name.asm" | tail -n +3  > "$pathstore$name.out"        
                fi
                cp "$name.out" "/tmp/a$name.out"
                echo "" >> "/tmp/a$name.out"
                d="$(diff "$pathstore$name.out" "$pathstore/a$name.out")"
                if [ -z "$d" ] ; then
